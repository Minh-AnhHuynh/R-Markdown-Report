...
stat_table <- make_kruskal_test(hallmark_mean_groups, "mean", "group_mean")
...
However _Hp Osteosarcoma_ in GSEA is not significantly differrent (p = 0.163).
...
```{r osteosarcoma_plot, fig.height=12, fig.width=15, fig.cap="**Heatmap of HP Osteosarcoma signature with samples annotated to their respective inflammatory groups.**"}

hm_ost %<>% as.ggplot()
hm_ost_c %<>% as.ggplot()
hm_os_type %<>% as.ggplot()
hm_os_type_mean %<>% as.ggplot()
pho <- ggarrange(hm_ost, hm_ost_c, hm_os_type, hm_os_type_mean, labels = "AUTO")
...
msigdb_path <-
  list.files("02_data/msigdb_signature/", pattern = "msig", full.names = TRUE)
combined_signature <-
  make_combined_signature(
    data = global_data,
    signature_list = "msigdbr",
    files = msigdb_path
  ) %>%
  rownames_to_column("sample")
hallmark_group <- hallmark_mean_groups %>%
  rename("cluster" = "group_mean") %>% # This is in order to use make_pheatmap
  select(sample, cluster) %>%
  left_join(combined_signature)

estimate_group <- hallmark_group %>% select(
  sample,
  cluster,
  "Immune_Score",
...
hm_estimate_mean <- make_pheatmap(
  estimate_group,
  clustering_method = "mds-kmeans",
  pheatmap_method = "unclustered_signature",
  pheatmap_signature = "combined",
  transpose = TRUE,
  cluster_column = FALSE,
  cluster_rows = FALSE,
  sample_cluster = 3,
...
estimate_table <- make_kruskal_test(estimate_group,
  quantvar = "Immune_Score",
  groupvar = "cluster"
)

```

```{r estimate_plot, fig.cap="ESTIMATE", fig.height=10, fig.width=15, cache=FALSE}
hm_estimate %<>% as.ggplot()
hm_estimate_mean %<>% as.ggplot()
est <- ggarrange(hm_estimate, hm_estimate_mean, estimate_table, labels = "AUTO")
suppressWarnings(print(est))
...
```{r mcp_counter, cache=TRUE, include=FALSE}
...
immune_plot <- mcp_plot / ciber_plot
immune_plot + plot_annotation(tag_levels = "A")

# cibersortx_tidy <- cibersortx %>%
#   tidyr::pivot_longer(cols = population, "population") %>%
#   select(1, population, value, group_mean) %>%
#   mutate(population = fct_reorder(population, value, mean),
#          group_mean = factor(cibersortx_tidy$group_mean,
#                              levels = c("Low", "Medium", "High")))
# 
# 
# 
...
# 
# 
# ggplot(cibersortx_tidy, aes(x = population, y = value, fill = group_mean)) +
#   geom_col(position = "dodge", colour = "black") + theme_pubr()
# 
# ggarrange(plotlist = bp, common.legend = TRUE)
# 
# 
# ggplot(cibersortx, aes(x = population, fill = group_mean))

```



```{r immune_plot, fig.height=20, fig.width=15, fig.cap= "Immune deconvolution plot. (A) MCP-counter", cache = TRUE}
# immune_plot <- ggarrange(mcp_plot, ciber_plot, labels = "AUTO")
...
#### Survival curve
```{r survival_code, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
...
```{r generate_raw_count, cache = TRUE}
...
```{r deseq2, cache=TRUE, include=FALSE}
...
```{r volc_plot, echo=FALSE, fig.cap="Volcano", fig.height=10, fig.width=10}
...
Using scRNA-Seq data from GSE152048, used by @Zhou.202005 and normalized by Sophie
