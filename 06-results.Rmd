...
librarian::shelf(
  ...
  snakecase, glue, rstatix, gridExtra, ggpmisc, ggplotify, patchwork, vroom
)
...
# The reason it is in a separated chunk is to account for working directory
# being set after the setup chunk.
...
In order to see if inflammatory groups associated to inflammation
response can be obtained, first an MDS visualization of the dataset on
the Hallmark Inflammatory Response signature (HIR) containing 200 genes,
from MSigDB **(Figure \@ref(fig:inflammatory-plot).A)**. The 88
osteosarcoma samples are overall clustered in the middle, while other
samples are scattered outwardly. The elbow method **(Figure\@ref(fig:inflammatory-plot).B)** seems to indicate that 2 clusters
seems to be the optimal number of clusters after which the wss decreases
the least. Visually, 8 clusters is the minimum amount of clusters for
which each point visually belongs to its nearest cluster. The gene
*ICAM4* was notably not detected in the dataset in the TARGET-OS cohort.
Thus, 199 genes from the signature were used.

However, due to the high amount of clusters and combined with a low
number of samples and the heterogeneity of the samples, another method
based on the intensity of the inflammation has been tried. By doing the
mean of the Z-score of the hallmark inflammatory response signature,
three groups describing the inflammatory status can be obtained,
classified as low, medium and high groups **(Figure \@ref(fig:inflammatory-plot).C)**.

Furthermore, the resulting groups are also partially functionally
relevant as they seem to correspond fairly well to the functional
groups, defined bythe k-means clustering based on MDS visualization
**(Figures \@ref(fig:inflammatory-plot).C, D)**. Each sample is thus
attributed to its inflammatory status and this group will be
subsequently used for the following results.

```{r inflammatory-plot, echo=FALSE, fig.cap="**Construction of inflammatory groups for the 88 samples in TARGET-OS dataset based on the HIR signature from MSigDB.** \\newline{}(A) MDS visualization with k-means clustering of the HIR signature. Data is Z-scaled. \\newline{}(B) Elbow graph determining the optimal number of k-clusters. \\newline{}(C) Histogram of the mean of Z-score of the HIR signature, annotated with the 8 k-means clusters. \\newline{}(D) Histogram of the mean of Z-score of the HIR signature, annotated with the 3 inflammatory status groups. \\newline{}(E) Heatmap of the HIR signature with samples annotated to their corresponding inflammatory groups. Rows correspond to genes and columns correspond to samples. Gene is expressed in TPM, Z-scaled. HIR = Hallmark Inflammatory Response. MDS = Multidimensional Scaling", fig.height=12, fig.width=10, dependson= "signature_construction"}
...
```{r hp_osteosarcoma, cache=TRUE, include=FALSE}
...
osteosarcoma_annotation <- get_osteosarcoma_type()[[2]] %>%
  filter(!type %in% c("TILs Markers", "Osteoclastic Markers", "Proliferation Markers")) %>%
  rownames_to_column("sample") %>%
  rename("Type" = type)
osteosarcoma_type <- get_osteosarcoma_type()[[1]] %>%
  select(osteosarcoma_annotation$sample) %>%
  rownames_to_column("sample")
osteosarcoma_type_mean_groups <-
  left_join(hallmark_mean_groups, osteosarcoma_type) %>%
  select(sample, "cluster" = group_mean, LUM:last_col())
...
between Low, Medium and High inflammation group **(Figure \@ref(fig:osteosarcoma-plot)A)**. However, the dendrogram clustering
the samples indicates that the gene signature does not cluster well with
the inflammatory annotations **(Figure \@ref(fig:osteosarcoma-plot)B)**. However, despite high heterogeneity
...
fibroblastic osteosarcomas which is expected **(Figure \@ref(fig:osteosarcoma-plot)C)**.
...
```{r estimate-violin, fig.asp=0.65, fig.align="center", fig.cap="**Violin plot of ESTIMATE score for each inflammatory group.** ANOVA was performed followed by Tukey's post-hoc analysis. $P \\leq 0.05$ is considered statistically significant."}
...
Immune cell abundance can be determined by various immune deconvolution
algorithm on TPM normalized bulk-RNASeq. Here, MCP-counter, CIBERSORTx
and xCell have been tried. In MCP-counter, the mean (SD) of Z-score
reflecting the abundances of neutrophils is statistically different
between all groups, it is increased when the inflammation status gets
higher **(Figure \@ref(fig:immune-plot)A)**
...
# MCP for neutrophils
source("01_script/09A_signature_function.R")
stat_table <- make_mean_comparison(mcp_counter, "Neutrophils", "group_mean", return_table = TRUE)
stat_dunn <- add_y_position(stat_table$dunn) %>%
  mutate(p.adj = format_p(.$p.adj))

# Only add y position, because x position is flawed. stat_pvalue_manual can get
# automatic position which is group1 and group2 by default.
...
mcpn_plot <- ggboxplot(
  ...
  fill = "group_mean",
  add = "mean_sd",
  palette = fill_color[[1]],
  ggtheme = theme_pubclean(),
  order = c("Low", "Medium", "High"),
  xlab = "",
  legend.title = "Group",
  ylab = "Mean (SD) of Z-score",
  subtitle = glue("Kruskal-Wallis, {format_p(stat_table$kruskal$p)}")
...
cibn_plot <- ggboxplot(
  ...
  legend.title = "Group",
  ylab = "Mean (SD) of Z-score",
  subtitle = glue("Kruskal-Wallis, {format_p(stat_krus$p)}")
)
...
```{r immune-plot, fig.cap="**Boxplot of the abundance of neutrophils with various immune deconvolution algorithm.** \\newline{}(A) MCP-counter. \\newline{}(B) CIBERSORTx. \\newline{}(C) xCell. \\newline{}Kruskal-Wallis testing is performed on non-parametric data and a post-hoc dunn's test was performed when appropriate. *P* $\\leq$ 0.05 is considered statistically significant.", fig.width=8, cache=TRUE, dependson=c("mcp_counter", "cibersortx", "xCell")}
immune_plot <-
  ggarrange(
    mcpn_plot,
    cibn_plot,
...
In order to determine if inflammation groups are linked with increased
survival odds, a Kaplan-Meier survival analysis was performed between
low and high group of inflammation **(Figure \@ref(fig:survival-plot))**. Log-rank analysis shows that the survival
curve between low and high group of inflammation are not statistically
different (*P* = 0.16). However the *P* value is low enough that it
could be considered a trend. Estimation considers 29 patients in the low
group and 28 patients in the high group, and estimates that only 9
patients remain alive in the low group while 20 patients are alive in
the high group.
...
survival_data <-
  vroom("02_data/GDC/GDC_clinical_data/clinical.tsv") %>%
...
# From ?Surv : The status indicator, 0=alive, 1=dead.
...
survival_data <-
  ...
  filter(!group_mean == 2) %>%
  write_tsv("03_results/survival_inflammatory_data.tsv")
...
```{r venn-plot, fig.asp=0.5, fig.cap = "**Venn diagram comparing three inflammatory signatures.**. ESTIMATE refers to the immune score calculated by the ESTIMATE algorithm, reflecting mmune cell infiltration. Hallmark Inflammatory Response and TNF-α signature are hallmark signatures taken from MSigDB."}
...
<!-- ```{r volc-plot, dependson=deseq2, echo=FALSE, out.width="60%", fig.align='center', fig.height=6, fig.width=6, dependson="deseq2", fig.cap="Volcano plot of the differentially expressed genes in Low vs High condition. Positive $log_2$ of fold change represent a gene that is more differentially expressed Low condition, and _vice versa_. Adjusted *P* value were obtained by FDR testing. *P* ≤ 0.05 and $log_2$ fold change ≤ 1 or ≥ 1 was used for cutoff. FDR = False Discovery Rate"} -->

<!-- suppressWarnings(print(volc_lh)) -->
<!-- ``` -->

A differential gene expression analysis was performed on the
inflammatory status condition (Low vs High) on the total 19114 genes.
<!-- A volcano plot **(Figure \@ref(fig:volc-plot))** represents the results of the analysis, with a slight skewness on the number of genes positively differentially expressed in the Low condition (right part of the graph). -->
DEGs are enriched through multiple databases in order to identify
potentially interesting pathways relating to a high level of
inflammation **(Figure \@ref(fig:enrichr-plot))**. Only relevant enrichment plots are shown. The combined enrichment barplot **(Figure \@ref(fig:enrichr-plot)A)** returned multiple pathways associated with chemokines, PD-1 signaling, and T cell lymphocyte associated mechanisms. Interestingly, enrichment of downregulated genes in GO Biological Process database returned a lot of pathways linked to immune cell chemotaxis, and particularly neutrophil chemotaxis.


```{r load_enrichr, include=FALSE}
# We load this package in an uncached chunk because we need it in order to
# automatically add it to package.bib and then cross-reference it in methods.
librarian::shelf(enrichR)
```

```{r enrichr, cache=TRUE, include=FALSE}
source("01_script/10_gene_enrichment.R")
ep <- plot_enrichR(
  dir_gene_list = "03_results/DESeq2_genes/",
  dir_figures = "04_figures/DESeq2_results/enrichr/",
  get_signature = "directory",
...
lhc <- ep$enriched_combined$`DEG_genes-DESeq2_group_mean_Low_vs_High_df`$combined_plot
lhg <- ep$plot_list$`downregulated_genes-DESeq2_group_mean_Low_vs_High_df`$GO_Biological_Process_2021
enrichp <- ggarrange(lhc, lhg, labels = "AUTO", nrow = 2, ncol = 1)
```


```{r enrichr-plot, echo=FALSE, dependson="enrichr", fig.width=12, fig.height=12, fig.cap="**Top 20 results of enrichment using enrichR for Low vs High condition.** \\newline{}(A) Combined enrichment for the 8 databases. \\newline{}(B) Enrichment for downregulated genes for GO Biological Process 2021 database."}

suppressWarnings(print(enrichp))
```
...
reported in the **Table \@ref(tab:gsea-table)**. Certain pathways are not significant but
relevant, such as "Gobp Positive Regulation of Vascular Associated
Smooth Muscle Cell Proliferation" (*P* = 0.129), "Gobp Positive
Regulation of Inflammatory Response" (*P* = 0.145), and all other pathways concerning
dendritic, lymphocyte, monocyte, macrophage chemotaxis and migration (*P* $\geq$ 0.150)
...
```{r gsea-table, dependson="gsea"}
gsea_df %>%
  filter(p.adjust <= 0.10) %>%
  dplyr::select(-qvalues) %>%
  mutate(p.adjust = insight::format_p(.$p.adjust)) %>%
  kableExtra::kbl(
    format = "latex",
    caption = glue("Results of GSEA by clusterProfiler, pre-ranked by descending order of log2 of fold change for genes differentially expressed in low vs high condition. Genes at the top of the list are thus differentially expressed in the high group, and vice versa. Only significant pathways are shown, out of {nrow(gsea_df)} selected pathways. $P$ adjust ≤ 0.10 is considered significant. NES = Normalized Enrichment Score"),
    digits = 3,
    booktabs = TRUE,
    # longtable = TRUE # Use argument to split table in multiple pages
  )
```

```{r write_packages, include=FALSE}
