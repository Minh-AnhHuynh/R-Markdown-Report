...
all Figure s, tables and statistical results (some of which may be
...
-   data should only be presented in the main text once, either in
    tables or Figure s; if presented in Figure s, data can be tabulated
    in appendices and referred to at the appropriate point in the main
    text.
...
samples are scattered outwardly. The elbow method **(Figure**
\@ref(fig:inflammatory-plot)**.B)** seems to indicate that 2 clusters
...
```{r estimate-violin, out.width="60%", fig.align='center',fig.cap="**Violin plot of ESTIMATE score for each inflammatory group.** ANOVA was performed followed by Tukey's post-hoc analysis. *P* $\\leq$ 0.05 is considered statistically significant."}
...
low and high group of inflammation **(Figure**
\@ref(fig:survival-plot)**)**. Log-rank analysis shows that the survival
