...
MDS visualization was performed with _vegan_'s metaMDS function, and clustering was effected with k-means clustering
...
data was performed between inflammatory groups (Low, Medium, High). Fold Change (FC) is calculated as $FC = High/Low$. DEGs were identified by using a cut-off on adjusted *P* value \le 0.05 and $log_2(FC) \ge 1 ; log_2(FC) \le -1.$ 
An interpretation
...
*P* \< 0.10 was considered statistically significant. *P* adjusted is calculated using the False Discovery Rate from Benjamini-Hochberg. 
