...
Using _DESeq2 (v1.34.0)_ [@Love.2014], standard DEG pipeline using raw data was performed between inflammatory groups (Low, Medium, High). DEGs were identified with _adjusted p.value (or FDR) $\le$ 0.05_ and _log2FoldChange $\ge$ 1 or _log2FoldChange $\le$ -1_. An interpretation of the FDR/Benjamini-Hochberg method for controlling the FDR is implemented in DESeq2 in which we rank the genes by p-value, then multiply each ranked p-value by m/rank (m = total number of tests).
...
## Statistical Testing
Statistical testing was performed using _R software 4.2.0 (2022-04-22)_. Kruskal-Wallis testing was performed for non-metric comparative analysis between groups. Post-hoc analysis was performed using Dunn's test as opposed to Wilcoxon due to the test taking into account Kruskal-Wallis's rank. P < 0.05 and P.adj < 0.05 was considered statistically significant.
